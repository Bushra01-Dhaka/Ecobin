...
const TotalModeratorTable = ({item, index, allModerator, setModerator}) => {
      ...
      if (result.isConfirmed) {
        axiosPublic.delete(`/users/${id}`).then((res) => {
          if (res.data.deletedCount > 0) {
            ...
            setModerator(allModerator?.filter((item) => item?._id !== id));
            Swal.fire({
              title: "Deleted!",
              text: "Moderator has been deleted.",
              icon: "success",
            });
          }
        });
         ...
         <tbody className="text-center border-0 border-b border-b-slate-300">
             <tr className="text-slate-800  font-semibold">
               ...
               <td>
                 ...
                 :
                 <button className="btn-sm p-1 rounded-md bg-green-700 text-white hover:bg-green-800">
                   Moderator
                 </button>}
               </td>
               <td>
...
export default TotalModeratorTable;